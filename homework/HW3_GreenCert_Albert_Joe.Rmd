...
K_folds = 5
green_folds = green %>%
  mutate(fold_id = rep(1:K_folds, length=nrow(green)) %>% sample)
...
The following shows the summary statistics for 'green_rating'. This model shows that holding all other variables constant, having a 'green_certification' increases the 'revenue per square foot per calendar year' by about \$1.42. 
...
stacked_data2 <- stacked_data2 %>% mutate(pred = -0.005778722 + 0.947066957*gbm_pred2 + 0.126333930*forest_pred2 - 0.073503965*lm_pred2)
